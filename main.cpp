...
int main()
{
    ...
    // le constructeur par défaut de client doit fonctionner (1)
    ...
    // le constructeur par paramètres de client doit fonctionner 2 
    ...
    // le constructeur par défaut de client premium doit fonctionner 3 
    ...
    // le constructeur par paramètres de client premium doit fonctionner 4
    ...
    // le constructeur par défaut de fournisseur doit fonctionner 5
    tests.push_back(mina.obtenirNom() == "Doe" &&
                    mina.obtenirPrenom() == "John" &&
                    mina.obtenirIdentifiant() == 0 &&
                    mina.obtenirCodePostal() == "A1A A1A");
    // le constructeur par paramètres de fournisseur doit fonctionner  6 
    ...
    // le constructeur par paramètres de produit aux enchères doit fonctionner 10
    ...
    // le constructeur par défaut de produit soldé doit fonctionner 11
    ...
    // le constructeur par paramètres de produit soldé doit fonctionner 12
    ...
    // une surenchère doit être strictement supérieure à l'offre précédente 13, 14 ,15
    ...
    // on ne peut pas ajouter le même produit deux fois dans le panier 24
    gaspard.ajouterProduit(&stylo); // produit normal
    gaspard.ajouterProduit(&nem);   // produit soldé
    tests.push_back(gaspard.obtenirPanier().size() == 2);
    // idem pour client premium 25
    julie.ajouterProduit(&stylo); // produit normal
    julie.ajouterProduit(&nem);   // produit soldé
    tests.push_back(julie.obtenirPanier().size() == 3);

    // ajouter le produit au fournisseur doit l'ajouter au catalogue 26, 27
    ...
    // modifier le fournisseur d'un produit doit ajouter le produit au nouveau fournisseur 28
    ...
    // modifier le fournisseur d'un produit doit retirer le produit du catalogue précédent 29
    tests.push_back(mina.obtenirCatalogue().size() == 0);
    // enlever un produit doit mettre l'attribut fournisseur à jour 30
    martine.enleverProduit(&montre);
    tests.push_back(montre.obtenirFournisseur() == nullptr);
    // enlever un produit doit le retirer du catalogue du fournisseur 31
    tests.push_back(martine.obtenirCatalogue().size() == 2 &&
                    martine.obtenirCatalogue()[0]->obtenirNom() == "chaussures" &&
                    martine.obtenirCatalogue()[1]->obtenirNom() == "violoncelle");
    // enlever un produit une deuxième fois ne doit rien faire 32
    martine.enleverProduit(&montre);
    tests.push_back(martine.obtenirCatalogue().size() == 2 &&
                    martine.obtenirCatalogue()[0]->obtenirNom() == "chaussures" &&
                    martine.obtenirCatalogue()[1]->obtenirNom() == "violoncelle");

    // réinitialiser doit vider le panier de tous les clients 33
    poly.reinitialiser();
    ...
    // réinitialiser doit mettre à jour l'attribut encherisseur_ quand nécessaire 34
    tests.push_back(violoncelle.obtenirEncherisseur() == nullptr);
    // réinitialiser doit remettre les produits aux enchères à leur prix intial 35
    tests.push_back(violoncelle.obtenirPrix() == violoncelle.obtenirPrixInitial());
    // réinitialiser doit vider le catalogue de tous les fournisseurs 36
    tests.push_back(martine.obtenirCatalogue().size() == 0 &&
                    samuel.obtenirCatalogue().size() == 0 &&
                    mina.obtenirCatalogue().size() == 0);
    // réinitialiser doit mettre à jour l'attribut fournisseur_ quand nécessaire 37
    tests.push_back(stylo.obtenirFournisseur() == nullptr &&
                    sculpture.obtenirFournisseur() == nullptr &&
                    nem.obtenirFournisseur() == nullptr);

    // le chiffre d'affaires devrait être nul après une réinitialisation 38
    tests.push_back(poly.obtenirChiffreAffaires() == 0.0);
    // le total à payer des clients normaux doit sommer les prix dans le panier 39
    ...
    // le total à payer des clients premiums doit prendre ne compte la réduction 40
    ...
    // la réduction ne doit pas induire des prix négatifs 41
    julie.ajouterProduit(&nem); // 0
    tests.push_back(julie.obtenirTotalAPayer() == 21095.0);
    // la surenchère doit actualiser le prix à payer pour l'ancien encherisseur 42
    tests.push_back(gaspard.obtenirTotalAPayer() == 110.0);
    // le total à payer des fournisseurs doit être nul 43
    samuel.ajouterProduit(&chaussures);
    samuel.ajouterProduit(&montre);
    tests.push_back(samuel.obtenirTotalAPayer() == 0.0);
    // le chiffre d'affaires doit additionner les totaux à payer 44
    tests.push_back(poly.obtenirChiffreAffaires() == 110.0 + 21095.0);
    
    // un usager ne peut pas être ajouté deux fois au gestionnaire 45, 46
